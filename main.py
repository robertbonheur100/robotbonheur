"""
╔══════════════════════════════════════════════════════════════╗
║      BONHEURBOT PRO v6 ELITE — PAT OTP FIX FINAL v4        ║
║       Multi-User Trading Bot — Deriv + Binance              ║
║                                                             ║
║  KÒREKSYON v4 (Properties not allowed: symbol):             ║
║   • connect_deriv_token: eseye WS authorize ANVAN REST      ║
║     PAT tokens mache ak WS authorize dirèk (PI FYAB)        ║
║   • place_trade WS fallback: "underlying" olye "symbol"     ║
║   • place_trade WS fallback: duration_unit "minutes"/"ticks"║
║   • place_digits_trade fallback: menm kòreksyon             ║
║   • api_connect: jere 2 chemen PAT (WS + REST)              ║
║                                                             ║
║  RÈGL SENP:                                                 ║
║   TOKEN KLASIK (REKÒMANDE) → WS ws.derivws.com              ║
║   PAT (pat_xxx) → eseye WS anvan → REST+OTP si echwe        ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def _build_rest_proposal(underlying, contract_type, stake, duration, duration_unit_ws,
    ...
    body = {
        ...
        "duration_unit": _rest_duration_unit(duration_unit_ws),
        "currency":      currency,
    }
...
# ═══════════════════════════════════════════════════════════
# ██  DERIV PAT REST CLIENT — OTP FIX FINAL v4  ██
# ═══════════════════════════════════════════════════════════
class DerivRESTClient:
    ...
    def _ws_call(self, build_msg_fn, check_done_fn, timeout=30):
        ...
        try:
            ...
            if not done.is_set():
                ...
                return None, f"Timeout {timeout}s — OTP WS pa reponn"
            return res[0], err[0]
        except Exception as e:
    ...
    def connect(self) -> float:
        ...
        raise Exception(
            "Token PAT echwe ak tout endpoints REST.\n\nDETAY ERÈ:\n" +
            "\n".join(errors) +
            "\n\nSOLISYON:\n"
    ...
    # ── Place Trade — KÒREKSYON v4 ────────────────────────
    def place_trade(self, symbol, direction, amount=1.0, duration_secs=60):
        """
        KÒREKSYON v4:
        - REST body: 'underlying' + 'stake' + duration_unit kòrèk
        - WS OTP fallback: 'underlying' olye 'symbol' (FIX Properties not allowed: symbol)
        - WS OTP fallback: duration_unit konvèti ("minutes" olye "m")
        """
        ...
        # KÒREKSYON v4: "underlying" olye "symbol" + duration_unit konvèti
        ...
        def send(ws):
            ws.send(json.dumps({
                ...
                "contract_type": ct,
                "currency":      "USD",
                "underlying":    symbol,                      # ← FIX v4: "underlying" olye "symbol"
                "duration":      dv,
                "duration_unit": _rest_duration_unit(du_ws), # ← FIX v4: "minutes" olye "m"
            }))
    ...
    # ── Place Digits Trade — KÒREKSYON v4 ────────────────
    def place_digits_trade(self, symbol, contract_type, amount=0.35, barrier=None):
        """
        KÒREKSYON v4:
        - REST body: 'underlying' + 'stake' + 'ticks'
        - WS OTP fallback: 'underlying' olye 'symbol' (FIX Properties not allowed: symbol)
        - WS OTP fallback: duration_unit "ticks" olye "t"
        """
        ...
        # ── Metòd 2: WS OTP fallback ──────────────────────
        # KÒREKSYON v4: "underlying" olye "symbol" + "ticks" olye "t"
        ...
        proposal_msg = {
            ...
            "underlying":    symbol,   # ← FIX v4: "underlying" olye "symbol"
            "duration":      5,
            "duration_unit": "ticks",  # ← FIX v4: "ticks" olye "t"
        }
        if barrier is not None:
...
# ═══════════════════════════════════════════════════════════
# WEBSOCKET AUTHORIZE — TOKEN KLASIK + PAT (FIX v4)
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# KÒREKSYON v4 — connect_deriv_token: WS anvan REST
# ═══════════════════════════════════════════════════════════
def connect_deriv_token(token: str, app_id: str = "1089"):
    if is_pat_token(token):
        # ── KÒREKSYON v4: eseye WS authorize ANVAN REST ────────────
        # PAT tokens Deriv aksepte authorize WS dirèk — PI FYAB
        # Evite tout erè REST ak OTP WS "Properties not allowed: symbol"
        logger.info(f"PAT → tante WS authorize standard anvan REST app_id={app_id}")
        ok, bal, lid, used_aid, note = connect_classic_token(token, app_id)
        if ok:
            logger.info(f"PAT WS ✓ loginid={lid} app_id={used_aid} ${bal:.2f}")
            return ok, bal, lid, used_aid, f"PAT (WS) | {lid} | app_id={used_aid}"

        # ── Fallback REST+OTP si WS echwe ──────────────────────────
        logger.info(f"PAT WS echwe ({note[:80]}) → tante REST+OTP api.derivws.com...")
        try:
            c   = DerivRESTClient(token, app_id)
            bal = c.connect()
            logger.info(f"PAT REST ✓ loginid={c.loginid} account_id={c._account_id} ${bal:.2f}")
...
def strat_deriv_pro_elite(c):
    ...
    def score_direction(up):
        ...
        _,pb=pivot_signal(c,"TRENDING_UP" if up else "TRENDING_DN"); s+=pb*10
        return s
    if trend_up:
...
class DerivDigitsClient:
    ...
    def connect(self):
        ...
        if self._is_pat and isinstance(used, DerivRESTClient):
            self._rest=used; self._rest._bal=bal
        elif not self._is_pat and used:
            self.app_id=used
        elif self._is_pat and isinstance(used, str):
            # PAT via WS — trète kòm klasik
            self._is_pat=False; self.app_id=used; self._rest=None
        return self._bal
    ...
    while st["running"]:
        ...
        try:
            ...
            try:
                ...
                if current_lot<mn*1.05: current_lot=round(mn*1.1,2)
            except: pass
            if st["balance"]<current_lot:
            ...
            try:
                ...
                add_log(st,f"✅ {sig} | ${current_lot:.2f} | SL:{SL_PCT*100:.1f}% TP:{TP_PCT*100:.1f}%","SUCCESS")
...
def trading_loop(st,bot_id=None):
    ...
    add_log(st,f"🚀 BonheurBot ELITE v6.4 | {symbol} | {strategy} | TF:{tf//60}min | Conf:{min_conf:.0%}")
    while st["running"]:
        ...
        try:
            ...
            st_sig,st_c=supertrend(candles); ha_sig,_=heikin_ashi_trend(candles)
            ...
            if consec_losses>=MAX_LOSSES:
                ...
                if not mache_bon: add_log(st,f"⏸ PÒZ APRE {consec_losses} PÈT | Tann {PAUSE}sek...","WARN"); time.sleep(PAUSE); continue
                else: add_log(st,f"✅ MACHE BON ANKÒ! {regime} ADX:{adx_val:.0f}","SUCCESS")
            if regime=="VOLATILE": add_log(st,f"⏸ VOLATILE — pa trade. Tann {min(tf,120)}sek...","WARN"); time.sleep(min(tf,120)); continue
            ...
            if ok:
                if pnl>0:
                    ...
                    if consec_losses<MAX_LOSSES:
                        ...
                        current_lot=max(0.5,min(round((total_lost+base_lot)/0.95,2),100.0))
                        add_log(st,f"🚨 3 PÈT AFILE! PÒZE OTOMATIK | ${current_lot:.2f}","WARN")
                trade={"id":len(st["trades"])+1,"time":datetime.now().strftime("%H:%M:%S"),
        ...
        time.sleep(tf)
    add_log(st,"⏹ BonheurBot ELITE v6.4 arrêté")
...
def api_connect():
    ...
    try:
        ...
        if broker == "deriv":
            ...
            # ── KÒREKSYON v4: jere 2 chemen PAT (WS oswa REST) ──────
            if is_pat_token(raw_token):
                if isinstance(client_obj, DerivRESTClient):
                    # Chemen REST+OTP
                    api_main            = client_obj
                    ...
                    add_log(st,
                        f"PAT REST+OTP ✓ account_id='{api_main._account_id}' "
                        f"loginid='{api_main._loginid}' ${balance:.2f}",
                        "SUCCESS")
                else:
                    # Chemen WS standard (PAT via authorize) — PI FYAB
                    ...
                    api_digits._bal     = balance
                    api_digits._is_pat  = False  # WS klasik

                    add_log(st,
                        f"PAT WS ✓ loginid='{loginid}' app_id={used_app_id} ${balance:.2f}",
                        "SUCCESS")
            else:
                used_app_id            = client_obj or app_id
                api_main               = DerivClient(raw_token, used_app_id)
...
<div id="login-page" style="display:none;min-height:100vh;background:#040A0F;align-items:center;justify-content:center;flex-direction:column">
  <div style="background:#071219;border:1px solid #0D2233;border-radius:12px;padding:40px;max-width:420px;width:90%;text-align:center">
    ...
    <div style="color:#4A7080;font-size:11px;margin-bottom:24px">Trading Bot v6.4 ELITE — PAT FIX v4</div>
...
<div class="hdr">
  <div style="display:flex;align-items:center;gap:12px">
    <div class="logo">💰 Bonheur<span>Bot</span> <span style="font-size:10px;color:#FFD600">ELITE v6.4</span></div>
    ...
    <div class="box">
      ...
      <div id="fd">
        <div style="background:#020C12;border:1px solid #0D2233;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="color:#00FF88;font-size:10px;font-weight:700;margin-bottom:8px">✅ PAT FIX v4 — WS anvan REST | underlying kòrèk</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            ...
            <div style="background:#FFD60010;border:1px solid #FFD60030;border-radius:6px;padding:8px">
              <div style="color:#FFD600;font-size:10px;font-weight:700;margin-bottom:4px">⚡ PAT (pat_xxx) — FIX v4</div>
              <div style="color:#4A7080;font-size:9px;line-height:1.8">1. Eseye WS authorize dirèk<br>2. Si echwe → REST+OTP<br>underlying+ticks kòrèk<br>developers.deriv.com</div>
            </div>
          </div>
        </div>
        <div class="iw">
      ...
      <div id="fb" style="display:none">
        ...
        <div id="fb-note" style="display:none;background:#FFD60010;border:1px solid #FFD60033;border-radius:6px;padding:8px;margin-bottom:8px;font-size:10px;color:#FFD600">🇺🇸 Binance US</div>
      </div>
...
<div id="pg-control" class="pg">
  <div class="g2">
    <div class="box">
      <div class="bt">PARAMÈT BOT ELITE v6.4</div>
        ...
        <div class="g2">
          ...
          <div class="iw"><div class="il">STRATEGY</div>
            <select id="c-st-forex">
              <option value="confluence">🔥 Confluence ELITE</option><option value="deriv_pro">🚀 Deriv Pro</option>
              <option value="supertrend">📈 SuperTrend</option><option value="heikin_ashi">🕯 Heikin Ashi</option>
              <option value="chandelier">🔔 Chandelier</option><option value="ai">🤖 AI Score</option>
              <option value="smc">🏛 Smart Money</option><option value="scalping_pro">⚡ Scalping Pro</option>
              <option value="ema">📊 EMA Classic</option><option value="rsi">📉 RSI Classic</option>
            </select>
          </div>
        </div>
      </div>
...
<div id="pg-admin" class="pg">
  <div class="stats">
    ...
    <div class="stat"><div class="sl">AKTIF</div><div class="sv" id="adm-aktif" style="color:#00FF88">—</div></div>
  ...
  <div class="g2">
    <div class="box">
      ...
      <div class="iw"><div class="il">TIP</div>
        <select id="new-code-type"><option value="user">👤 Itilizatè — 1 mwa</option><option value="adm">👑 Admin — pa ekspire</option></select>
      </div>
...
function showLogin(err=""){
  document.getElementById("login-page").style.display="flex";document.getElementById("app-page").style.display="none";
  if(err&&err!=="Pa gen sesyon"&&err!=="Mete kòd aksè ou a")document.getElementById("login-err").innerHTML=`<div class="al er">⚠ ${err}</div>`;
}
function showApp(msg){document.getElementById("login-page").style.display="none";document.getElementById("app-page").style.display="block";document.getElementById("sub-info").textContent=msg||"";}
...
function autoDetectToken(){
  ...
  if(val.startsWith("pat_")){
    badge.style.display="inline";badge.className="bp";badge.textContent="⚡ PAT→WS d'abord→REST fallback";
    hint.innerHTML='<span style="color:#FFD600">⚡ PAT v4: eseye WS authorize anvan → REST+OTP si echwe</span>';
...
async function doConn(){
  ...
  if(br==="deriv"){
    ...
    msg("cm",`⏳ ${isPat?"PAT v4 → WS authorize anvan → REST fallback":"Klasik → WS app_id="+appId} | Ap konekte...`,"ok");
  }
  ...
  try{
    ...
    if(d.ok){
      ...
      if(d.token_type){const hb=document.getElementById("h-tok-type");hb.style.display="inline";hb.className=d.token_type==="PAT"?"bp":"bk";hb.textContent=d.token_type==="PAT"?"⚡ PAT-v4":"✅ Klasik-WS";}
...
async function doStart(){
  ...
  const d=await r.json();
  if(d.ok){msg("ctm","✅ BonheurBot ELITE v6.4 démarre!","ok");document.getElementById("bs").style.display="none";document.getElementById("bx").style.display="inline-block"}
  else msg("ctm","✗ "+d.error,false);
}
async function doStop(){
  await fetch("/api/stop",{method:"POST"});msg("ctm","✅ Bot arrêté","ok");
...
const SI={
  confluence:{l:"🔥 Confluence ELITE",d:"SuperTrend+HeikinAshi+Chandelier+10 strategies. ADX≥12, 3+ strat.",tags:["SuperTrend","HeikinAshi","ADX≥12"]},
  deriv_pro:{l:"🚀 Deriv Pro ELITE",d:"Score 5/15 + ADX≥12 + SuperTrend.",tags:["score 5/15","ADX≥12"]},
  supertrend:{l:"📈 SuperTrend",d:"ATR×3. Siyal klè BUY/SELL.",tags:["ATR×3","75-92%"]},
  ...
  ai:{l:"🤖 AI Score",d:"8 faktè: EMA+RSI+MACD+BB+momentum.",tags:["8 faktè","68-92%"]},
  smc:{l:"🏛 SMC",d:"Break of Structure + swing + EMA50.",tags:["BOS","EMA50"]},
  scalping_pro:{l:"⚡ Scalping",d:"EMA 5/13 + RSI 9.",tags:["EMA 5/13","RSI 9"]},
  rsi:{l:"📉 RSI",d:"RSI <30/>70 + EMA50.",tags:["RSI 14"]},
  binance_gold:{l:"🥇 Gold",d:"XAU: EMA+RSI+MACD+Volume.",tags:["EMA 20/50/200"]},
  binance_crypto:{l:"🪙 Crypto",d:"Binance: Trend+Volume+RSI+MACD.",tags:["EMA 9/21/50"]},
};
let sel="confluence";
...
function renderS(){const s=SI[sel];document.getElementById("sdet").innerHTML=`<div class="bt">${s.l}</div><div style="color:#C8E8F0;line-height:1.8;margin-bottom:12px">${s.d}</div><div style="display:flex;gap:8px;flex-wrap:wrap">${s.tags.map(t=>`<span class="tag" style="border-color:#FFD60044;color:#FFD600">${t}</span>`).join("")}</div>`;}
renderS();
...
function upd(d){
  ...
  document.getElementById("hb").textContent=blab;document.getElementById("hb").style.color=d.connected?"#00FF88":"#3A6070";
  ...
  document.getElementById("s-sent").textContent="$"+d.profit_sent.toFixed(4);document.getElementById("c-sent").textContent="$"+d.profit_sent.toFixed(4);
  ...
  const br2=document.getElementById("s-br2");if(br2){br2.textContent=blab;br2.style.color=d.connected?"#00FF88":"#3A6070"}
...
async function admRefresh(){
  ...
  try{
    ...
    if(d2.ok){
      ...
      document.getElementById("adm-users-list").innerHTML=d2.total===0?'<div style="color:#3A6070;text-align:center;padding:20px">Pa gen itilizatè</div>'
  ...
  try{
    ...
    if(d3.ok){document.getElementById("adm-sessions-list").innerHTML=d3.sessions.length===0?'<div style="text-align:center;padding:10px">Pa gen sesyon</div>'
...
async function admAddCode(){
  ...
  const d=await r.json();document.getElementById("add-code-msg").innerHTML=`<div class="al ${d.ok?"ok":"er"}">${d.ok?d.msg:d.error}</div>`;
...
if __name__ == "__main__":
    ...
    logger.info(f"BonheurBot ELITE v6.4 — PAT FIX v4 (WS anvan REST | underlying+ticks) — port {port}")
