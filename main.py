...
║                                                             ║
║  VRÈ DERIV REST API (developers.deriv.com konfime):        ║
...
║   2. POST /trading/v1/options/accounts/{id}/otp → ws_url   ║
║   3. Connect WS URL (deja otantifye — pa bezwen authorize) ║
║                                                             ║
║  TOKEN KLASIK → WebSocket + {"authorize": "token"}         ║
║  TOKEN PAT    → REST Bearer → OTP → WebSocket otantifye    ║
║                                                             ║
...
# ═══════════════════════════════════════════════════════════
# VRÈ DERIV REST API CONSTANTS (konfime sou developers.deriv.com)
# ═══════════════════════════════════════════════════════════
DERIV_REST_BASE = "https://api.derivws.com/trading/v1"
DERIV_WS_APP_IDS = ["1089", "36544", "16929"]

# WebSocket endpoints (pou token klasik)
DERIV_WS_PUBLIC   = "wss://api.derivws.com/trading/v1/options/ws/public"
DERIV_WS_DEMO     = "wss://api.derivws.com/trading/v1/options/ws/demo"
DERIV_WS_REAL     = "wss://api.derivws.com/trading/v1/options/ws/real"
DERIV_WS_LEGACY   = "wss://ws.derivws.com/websockets/v3"  # pou token klasik
...
# ═══════════════════════════════════════════════════════════
# ██  VRÈ PAT FIX — DERIV NEW REST API  ██
#
#  DOKIMANTASYON OFISYÈL (developers.deriv.com):
#   BASE: https://api.derivws.com/trading/v1/
#   AUTH: Deriv-App-ID + Authorization: Bearer pat_xxx
#
#  WORKFLOW PAT:
#   1. GET  /options/accounts           → jwenn accountId
#   2. POST /options/accounts/{id}/otp  → jwenn ws_url otantifye
#   3. Connect ws_url (pa bezwen authorize)
#   4. Voye commandes WS nòmal: proposal, buy, balance, etc.
#
#  TOKEN KLASIK:
#   → WebSocket wss://ws.derivws.com/websockets/v3?app_id=1089
#   → {"authorize": "token_klasik"}
#   → Apre: voye commandes nòmal
#
# ═══════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────────────────────────
# DERIV PAT REST CLIENT — Vrè API endpoints
# ─────────────────────────────────────────────────────────────
class DerivRESTClient:
    """
    PAT Token client itilize vrè Deriv REST API:
    https://api.derivws.com/trading/v1/

    Workflow:
    1. GET /options/accounts → accountId + balance
    2. POST /options/accounts/{id}/otp → ws_url otantifye
    3. WebSocket connections via ws_url (deja otantifye)
    """
    ...
    def __init__(self, pat_token: str, app_id: str = "1089", timeout: int = 25):
        ...
        self._ws_demo_url = None
        self._ws_real_url = None
        self._headers = {
    ...
    def _get(self, path, params=None):
        ...
        logger.debug(f"GET {url} → {r.status_code}: {r.text[:200]}")
        r.raise_for_status()
        return r.json()
    ...
    def connect(self) -> float:
        """
        Otantifye PAT via GET /options/accounts
        Lè sa pa fonksyone, eseye OTP endpoint pou jwenn WS URL
        Retounen balance kòman float
        """
        errors = []

        # ── Etap 1: Jwenn tout comptes ──
        try:
            data = self._get("/options/accounts")
            logger.info(f"PAT /options/accounts response: {str(data)[:300]}")

            # Response ka se yon liste oswa yon dikt ak 'accounts'
            accounts = []
            if isinstance(data, list):
                accounts = data
            elif isinstance(data, dict):
                accounts = data.get("accounts") or data.get("data") or []
                # Pafwa response dirèk gen balance
                if "balance" in data:
                    self._bal = float(data["balance"])
                    self._loginid = data.get("loginid") or data.get("account_id") or "PAT_USER"
                    return self._bal

            if accounts:
                # Pran premye compte reyèl, oswa premye compte
                real_acc = next((a for a in accounts if a.get("account_type") == "real"), accounts[0])
                self._account_id = real_acc.get("account_id") or real_acc.get("id")
                self._loginid = real_acc.get("loginid") or self._account_id or "PAT_USER"
                self._bal = float(real_acc.get("balance", 0) or 0)

                # Eseye jwenn WS URL via OTP
                if self._account_id:
                    try:
                        otp_data = self._post(f"/options/accounts/{self._account_id}/otp")
                        logger.info(f"OTP response: {str(otp_data)[:200]}")
                        ws_url = (otp_data.get("url") or otp_data.get("ws_url") or
                                  otp_data.get("websocket_url") or "")
                        if ws_url:
                            if "demo" in ws_url.lower():
                                self._ws_demo_url = ws_url
                            else:
                                self._ws_real_url = ws_url
                            logger.info(f"PAT WS URL: {ws_url[:80]}")
                    except Exception as e:
                        logger.warning(f"OTP request echwe: {e}")

                logger.info(f"PAT OK | {self._loginid} | ${self._bal:.2f} | acct={self._account_id}")
                return self._bal

        except requests.HTTPError as e:
            code = e.response.status_code
            body = e.response.text[:200]
            errors.append(f"GET /options/accounts: HTTP {code} → {body}")
            logger.warning(f"PAT /options/accounts echwe: {code} {body}")
        except Exception as e:
            errors.append(f"GET /options/accounts: {str(e)[:150]}")
            logger.warning(f"PAT /options/accounts erè: {e}")

        # ── Etap 2: Eseye kreye compte demo si pa gen compte ──
        try:
            data = self._post("/options/accounts", {"currency": "USD", "group": "row", "account_type": "demo"})
            ...
            if data and "errors" not in data:
                ...
                self._bal = float(data.get("balance", 0) or 0)
                if self._account_id:
                    return self._bal
        except Exception as e:
            errors.append(f"POST /options/accounts: {str(e)[:100]}")

        # ── Etap 3: Health check pou konfime token valid ──
        try:
            data = self._get("/health")
            logger.info(f"Health check: {str(data)[:100]}")
            self._loginid = "PAT_USER"
            self._bal = 0.0
            logger.warning("PAT: health OK men balance unavailable")
            return 0.0
        except Exception as e:
            errors.append(f"GET /health: {str(e)[:80]}")

        # ── Etap 4: Sistème pi vye (legacy) ──
        try:
            legacy_headers = dict(self._headers)
            legacy_headers["Authorization"] = f"Bearer {self.token}"
            r = requests.get("https://api.deriv.com/v2/account",
                             headers=legacy_headers, timeout=15)
        ...
        raise Exception(
            f"Token PAT echwe ak tout endpoints.\n\nDETAY ERÈ:\n" +
            "\n".join(errors) +
            "\n\n" + "─" * 50 + "\n"
            "SOLISYON 1 — ITILIZE TOKEN KLASIK (Rekòmande):\n"
            "  app.deriv.com → foto ou → API Token\n"
            "  Create new token → ✓ Read ✓ Trade ✓ Payments\n"
            "  Kole token (PA kòmanse ak pat_) — App ID: 1089\n\n"
            "SOLISYON 2 — PAT via nouvo Deriv Dashboard:\n"
            "  developers.deriv.com → Dashboard → API tokens\n"
            "  Kreye PAT ak scope 'trade' + 'account_manage'\n"
            "  App ID nesese pou REST API"
        )

    def _get_ws_url(self) -> str:
        """Jwenn WS URL otantifye (reyèl oswa demo)"""
        if self._ws_real_url: return self._ws_real_url
        if self._ws_demo_url: return self._ws_demo_url
        # Eseye jwenn via OTP si pa gen URL
        if self._account_id:
            try:
                otp_data = self._post(f"/options/accounts/{self._account_id}/otp")
                url = (otp_data.get("url") or otp_data.get("ws_url") or
                       otp_data.get("websocket_url") or "")
                if url:
                    if "demo" in url.lower(): self._ws_demo_url = url
                    else: self._ws_real_url = url
                    return url
            except: pass
        return ""

    def _ws_send_recv(self, message_dict, timeout=25):
        """
        Voye yon message WS ak connection otantifye (PAT OTP URL)
        oswa WebSocket legacy ak Bearer token
        """
        import websocket as wsl
        ws_url = self._get_ws_url()

        if ws_url:
            # Nouvo API: WS URL deja otantifye — pa bezwen authorize
            ...
            def on_err(ws, e):
                if not done.is_set():
                    res[0] = {"error": {"message": str(e)}}; done.set()

            def on_open(ws):
                ws.send(json.dumps(message_dict))

            try:
                ws = wsl.WebSocketApp(ws_url, on_open=on_open, on_message=on_msg, on_error=on_err)
                ...
                logger.warning(f"PAT WS (new API) echwe: {e}")

        # Fallback: pa gen WS URL — retounen empty dict
        return {}
    ...
    def get_balance_sync(self) -> float:
        try:
            ...
            if accounts:
                ...
                bal = float(real.get("balance", 0) or 0)
                if bal > 0: self._bal = bal; return bal
        except: pass
        return self._bal

    def get_candles(self, symbol="R_100", count=200, gran=60):
        """Jwenn chandèl via WS (PAT OTP URL oswa legacy)"""
        try:
            ws_url = self._get_ws_url()
            if ws_url:
                import websocket as wsl
            ...
            logger.warning(f"PAT get_candles (new WS): {e}")

        # Fallback via public WS (pa bezwen auth pou ticks_history)
        try:
            import websocket as wsl
            res = [None]; done = threading.Event()
            def on_msg(ws, msg):
            ...
            # Public endpoint — pa bezwen auth pou ticks_history
            ...
            logger.error(f"PAT get_candles fallback: {e}")
        return []

    def get_ticks(self, symbol="R_10", count=100):
        """Jwenn ticks via public WS (pa bezwen auth)"""
        try:
            import websocket as wsl
            res = [None]; done = threading.Event()
            def on_msg(ws, msg):
                d = json.loads(msg)
                if d.get("msg_type") == "history": res[0] = d.get("history", {}); done.set(); ws.close()
                elif "error" in d: done.set(); ws.close()
            def on_open(ws):
            ...
            if res[0]:
                prices = res[0].get("prices", [])
                times  = res[0].get("times",  [])
                return [{"price": float(p), "time": t} for p, t in zip(prices, times)]
        except Exception as e:
            logger.error(f"PAT get_ticks: {e}")
        return []

    def place_trade(self, symbol, direction, amount=1.0, duration_secs=60):
        """Place trade via WS URL otantifye (PAT OTP)"""
        ws_url = self._get_ws_url()
        if not ws_url:
            raise Exception(
                "Pa ka jwenn WS URL otantifye.\n"
                "Asire ou konekte kòrèkteman ak PAT token.\n"
                "Rekòmandèr: itilize Token Klasik (pa pat_xxx) pou trading."
            )
        ...
        def on_open(ws):
            ws.send(json.dumps({
                "proposal": 1, "amount": max(0.5, float(amount)), "basis": "stake",
                "contract_type": ct, "currency": "USD", "symbol": symbol,
                "duration": dv, "duration_unit": du
            }))

        def on_err(ws, e):
            if not done.is_set(): err[0] = str(e); done.set()
    ...
    def place_digits_trade(self, symbol, contract_type, amount=0.35, barrier=None):
        ws_url = self._get_ws_url()
        if not ws_url:
            raise Exception("Pa ka jwenn WS URL otantifye pou digits trade.")

        import websocket as wsl
        proposal_msg = {
            "proposal": 1, "amount": max(0.35, float(amount)), "basis": "stake",
        ...
        def on_open(ws): ws.send(json.dumps(proposal_msg))
        def on_err(ws, e):
            if not done.is_set(): err[0] = str(e); done.set()
    ...
    def transfer_to_account(self, account_id, amount):
        ws_url = self._get_ws_url()
        if not ws_url: raise Exception("Pa gen WS URL pou transfer")
...
def connect_deriv_token(token: str, app_id: str = "1089"):
    """
    ROUTER PRENSIPAL:
      PAT (pat_xxx) → DerivRESTClient.connect()   [REST Bearer + OTP WebSocket]
      KLASIK        → connect_classic_token()      [WebSocket authorize]
    Retounen: (ok, balance, loginid, used_app_id, note)
    """
    if is_pat_token(token):
        logger.info(f"PAT token → Deriv REST API Bearer (api.derivws.com) app_id={app_id}")
        try:
...
def calc_pivot_points(candles):
    ...
    recent = candles[-20:]; hi = max(x["high"] for x in recent)
    lo = min(x["low"] for x in recent); cl = candles[-1]["close"]
...
# ═══════════════════════════════════════════════════════════
# DERIV DIGITS CLIENT — DUAL (PAT=REST+OTP / KLASIK=WS)
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# TRADING LOOPS (enchangé — menm logik, jis API clients korije)
# ═══════════════════════════════════════════════════════════
...
def trading_loop(st, bot_id=None):
    ...
    while st["running"]:
        ...
        try:
            ...
            if regime == "VOLATILE":
                add_log(st, f"⏸ VOLATILE — pa trade. Tann {min(tf, 120)}sek...", "WARN")
...
def api_connect():
    ...
    try:
        ...
        if broker == "deriv":
            ...
            method_note = "REST API (api.derivws.com) + OTP WebSocket" if tok_type == "PAT" else f"WebSocket ws.derivws.com app_id={app_id}"
            add_log(st, f"🔑 {tok_type} token → {method_note}", "INFO")
            ok, balance, loginid, used_app_id, note = connect_deriv_token(raw_token, app_id)
            if not ok:
                if tok_type == "PAT":
                    err = (f"❌ Token PAT echwe\n\n{note}\n\n"
                           f"SOLISYON REKÒMANDE — ITILIZE TOKEN KLASIK:\n"
                           f"1. app.deriv.com → foto ou → API Token\n"
                           f"2. Create new token → ✓ Read ✓ Trade ✓ Payments\n"
                           f"3. Kole token (PA kòmanse ak pat_) — App ID: 1089\n\n"
                           f"SI OU VLE ITILIZE PAT:\n"
                           f"1. developers.deriv.com → Dashboard → API tokens\n"
                           f"2. Kreye PAT ak scope: trade + account_manage\n"
                           f"3. Anrejistre app ak PAT type (pa OAuth)")
                else:
...
# ═══════════════════════════════════════════════════════════
# HTML INTERFACE — v6 ELITE (menm UI, just update notes)
# ═══════════════════════════════════════════════════════════
HTML = r"""<!DOCTYPE html>
<html>
<head>
    ...
    <div class="box">
      ...
      <div id="fd">
        ...
        <div style="background:#020C12;border:1px solid #0D2233;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="color:#00FF88;font-size:10px;font-weight:700;margin-bottom:8px">✅ DUAL CLIENT — VRÈ DERIV API</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div style="background:#00FF8810;border:1px solid #00FF8830;border-radius:6px;padding:8px">
              ...
              <div style="color:#4A7080;font-size:9px;line-height:1.8">PA kòmanse ak <code>pat_</code><br>→ WebSocket ws.derivws.com<br>→ {"authorize": "token"}<br>App ID: <b>1089</b><br>Pèmisyon: Read+Trade+Pay</div>
            </div>
            <div style="background:#FFD60010;border:1px solid #FFD60030;border-radius:6px;padding:8px">
              ...
              <div style="color:#4A7080;font-size:9px;line-height:1.8">Kòmanse ak <code>pat_</code><br>→ REST api.derivws.com<br>→ Bearer + Deriv-App-ID<br>→ OTP → WS URL otantifye<br>developers.deriv.com</div>
            </div>
          </div>
        </div>
        <div class="iw">
        ...
        <div class="iw" id="appid-row">
          <div class="il">APP ID <span id="appid-note" style="color:#4A7080">(Token Klasik = 1089 / PAT = App ID ou a)</span></div>
          <input id="d-ai" value="1089" placeholder="1089">
          <div id="appid-hint" style="color:#4A7080;font-size:9px;margin-top:3px">Token Klasik: <b>1089</b> (defot) • PAT: App ID nan developers.deriv.com</div>
        </div>
        <div style="background:#020C12;border:1px solid #0D2233;border-radius:6px;padding:10px;font-size:10px;line-height:1.9;color:#4A7080">
          <div style="color:#00FF88;font-weight:700;margin-bottom:4px">📖 KREYE TOKEN KLASIK (Pi Fasil):</div>
          app.deriv.com → foto ou → API Token → Create new token<br>
          Pèmisyon: ✓ Read ✓ Trade ✓ Payments → App ID: 1089<br><br>
          <div style="color:#FFD600;font-weight:700;margin-bottom:4px">📖 TOKEN PAT (pat_xxx) — Pou Devlopè:</div>
          developers.deriv.com → Dashboard → API tokens<br>
          Kreye PAT + scope trade+account_manage + enrejistre app PAT type
        </div>
      </div>
...
function autoDetectToken(){
  ...
  if(val.startsWith("pat_")){
    badge.style.display="inline"; badge.className="badge-p"; badge.textContent="⚡ PAT → REST+OTP";
    hint.innerHTML='<span style="color:#FFD600">⚡ PAT détecté → REST api.derivws.com + OTP WebSocket. App ID = ID nan developers.deriv.com</span>';
    appHint.innerHTML='<span style="color:#FFD600">PAT: mete App ID ou a nan developers.deriv.com (pa 1089)</span>';
    appNote.innerHTML='<span style="color:#FFD600">(PAT: App ID ou a)</span>';
  }else if(val.length>10){
    badge.style.display="inline"; badge.className="badge-k"; badge.textContent="✅ KLASIK → WebSocket";
    hint.innerHTML='<span style="color:#00FF88">✅ Token Klasik → WebSocket ws.derivws.com + {"authorize"}. App ID: 1089!</span>';
    appRow.style.opacity="1";
    appHint.innerHTML='Token Klasik: <b>1089</b> (defot — bon!)';
    appNote.innerHTML='(Token Klasik = 1089 / PAT = App ID ou a)';
  }else{
    badge.style.display="none";
    hint.innerHTML='Token Klasik PA kòmanse ak <code>pat_</code>';
    appHint.innerHTML='Token Klasik: <b>1089</b> (defot) • PAT: App ID nan developers.deriv.com';
    appNote.innerHTML='(Token Klasik = 1089 / PAT = App ID ou a)';
  }
}
...
async function doConn(){
  ...
  if(br==="deriv"){
    ...
    msg("cm",`⏳ ${isPat?"PAT → REST api.derivws.com + OTP WebSocket":"Klasik → WebSocket ws.derivws.com app_id="+appId} | Ap konekte...`,"ok");
  }
...
if __name__ == "__main__":
    ...
    logger.info(f"BonheurBot ELITE v6 — VRÈ PAT FIX (REST api.derivws.com + OTP WebSocket) — port {port}")
