...
"""
╔══════════════════════════════════════════════════════════════╗
║     BONHEURBOT PRO v6 ELITE — PAT PnL + TRADE FIX FINAL    ║
║      Multi-User Trading Bot — Deriv + Binance               ║
║                                                             ║
║  FIX KORIJE:                                                ║
║   ✅ PAT PnL — sèvi sell_price/buy_price reyèl              ║
║   ✅ PAT trade — bal_before pran ANVAN trade                ║
║   ✅ PAT wait_contract — parse status kòrèk                 ║
║   ✅ PAT limit profit/pèt — respekte kòrèkteman             ║
║   ✅ PAT get_candles — timeframe kòrèk                      ║
║   ✅ underlying_symbol PAT / symbol Klasik                  ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# ═══════════════════════════════════════════════════════════
# ██  DERIV PAT REST CLIENT — PnL FIX KONPLÈ  ██
# ═══════════════════════════════════════════════════════════
class DerivRESTClient:
        ...
        try:
            ...
            if isinstance(resp, dict):
                ...
                if not ws_url and isinstance(resp.get("data"), str):
                    ws_url = resp["data"]
            if ws_url:
                logger.info(f"PAT OTP URL: {ws_url[:120]}")
                return ws_url
            else:
                logger.error(f"PAT OTP: pa jwenn URL. Response: {resp}")
                return ""
    ...
    def connect(self) -> float:
        ...
        try:
            ...
            logger.info(f"PAT accounts jwenn: {len(accounts)}")
            ...
            if accounts:
                ...
                logger.info(f"PAT account_id='{self._account_id}' | ${self._bal:.2f}")
                if self._account_id:
                    self._fresh_ws_url()
                    return self._bal
        except requests.HTTPError as e:
            errors.append(f"GET /options/accounts: HTTP {e.response.status_code}")
            logger.error(f"PAT /options/accounts HTTP: {e.response.status_code}")
        except Exception as e:
            errors.append(f"GET /options/accounts: {str(e)[:200]}")
            logger.error(f"PAT /options/accounts: {e}")
        try:
            self._get("/health")
            return 0.0
        except Exception as e:
    ...
    # ─── FIX: get_balance_sync reyèl pou PAT ───
    def get_balance_sync(self) -> float:
        try:
            data = self._get("/options/accounts")
            accs = self._extract_accounts(data)
            if accs:
                real = next((a for a in accs if str(a.get("account_type","")).lower() in ("real","financial","gaming")), accs[0])
            ...
            logger.warning(f"PAT get_balance_sync: {e}")
        return self._bal
 
    def get_candles(self, symbol="R_100", count=200, gran=60):
        # Eseye via OTP WS anvan, sinon public WS
        if self._account_id:
            def send(ws):
    ...
    def place_trade(self, symbol, direction, amount=1.0, duration_secs=60):
        ...
        def send(ws):
            proposal_msg = {
                ...
                "contract_type":     ct,
                "currency":          "USD",
                "underlying_symbol": symbol,   # ✅ FIX PAT
                "duration":          dv,
                "duration_unit":     du,
            }
    ...
    # ─── FIX KRITIS: wait_contract_result — parse PnL kòrèk ───
    def wait_contract_result(self, contract_id, timeout=35):
        """
        Retounen dict ki gen:
          status: "won" | "lost" | "sold"
          buy_price: montan stake reyèl
          sell_price: montan resevwa (0 si pèdi)
          profit: pnl reyèl (sell_price - buy_price)
        """
        def send(ws):
        ...
        def recv(d, ws, res, err, done):
            ...
            if mt == "proposal_open_contract":
                ...
                if status in ("won", "lost", "sold"):
                    ...
                    profit     = float(poc.get("profit", sell_price - buy_price))
                    poc["_parsed_buy_price"]  = buy_price
                    poc["_parsed_sell_price"] = sell_price
                    poc["_parsed_profit"]     = profit
                    res[0] = poc; done.set()
                    try: ws.close()
                    except: pass
            elif "error" in d:
                err[0] = d["error"].get("message", "erè"); done.set()
...
# ═══════════════════════════════════════════════════════════
# WEBSOCKET AUTHORIZE — TOKEN KLASIK
# ═══════════════════════════════════════════════════════════
...
def strat_confluence_elite(c, min_strats=3, min_per_conf=0.65):
    ...
    NW = 2.5
    for sig, conf, w in [(st_sig,st_conf,NW),(ha_sig,ha_conf,NW),(ce_sig,ce_conf,NW),(vw_sig,vw_conf,1.8)]:
    ...
    if regime == "RANGING":
        ...
        if buy_new>=2 and buy_cnt>=min_strats and buy_score>sell_score*dom_ratio:
            ip,pb = pivot_signal(c,"TRENDING_UP")
        ...
        if sell_new>=2 and sell_cnt>=min_strats and sell_score>buy_score*dom_ratio:
            ip,pb = pivot_signal(c,"TRENDING_DN")
...
def strat_deriv_pro_elite(c):
    ...
    macd_hist=m-sig_
    m_prev,sig_prev=macd(cl[:-1]) if len(cl)>=2 else (m,sig_)
    ...
    trend_up = e9[-1]>e21[-1] and (not e50 or cl[-1]>e50[-1]*0.998)
    trend_down= e9[-1]<e21[-1] and (not e50 or cl[-1]<e50[-1]*1.002)
    ...
    if trend_up:
        ...
        if ip: score+=1.5
        if score>=5.0:
            conf=min(0.95,0.76+(score/15.0)*0.25)
...
def strat_binance_crypto(c):
    ...
    buy_pts=0; sell_pts=0
    if e9[-1]>e21[-1]>e50[-1]: buy_pts+=3+(2 if long_bull else 0)
    if e9[-1]<e21[-1]<e50[-1]: sell_pts+=3+(2 if long_bear else 0)
    if len(e9)>=2:
        if e9[-2]<e21[-2] and e9[-1]>e21[-1]: buy_pts+=3
        if e9[-2]>e21[-2] and e9[-1]<e21[-1]: sell_pts+=3
    ...
    if cl[-1]>hi20 and cl[-2]<=hi20:
        buy_pts+=(3 if curr_vol>avg_vol*1.5 else 1)
    if cl[-1]<lo20 and cl[-2]>=lo20:
        sell_pts+=(3 if curr_vol>avg_vol*1.5 else 1)
    st_sig,_=supertrend(c,p=10,mult=3.0)
    if st_sig=="BUY": buy_pts+=2
    if st_sig=="SELL": sell_pts+=2
...
def strat_confluence_binance(c, symbol="BTCUSDT"):
    ...
    if confirm>=3:
        return primary, max(0.75, min(0.92, total_conf/(confirm+2)))
    return "NONE",0
...
class DerivDigitsClient:
    ...
    def wait_contract_result(self,contract_id,timeout=35):
        if self._is_pat and self._rest:
            return self._rest.wait_contract_result(contract_id,timeout)
        import websocket as wsl
        res=[None]; done=threading.Event()
        def on_msg(ws,msg):
            d=json.loads(msg); mt=d.get("msg_type","")
...
def digits_trading_loop(st,bot_id=None):
    ...
    while st["running"]:
        ...
        try:
            ...
            # ─── FIX: pran balans ANVAN trade ───
            bal_before = api.get_balance_sync()
            if not bal_before or bal_before <= 0:
                bal_before = st["balance"]
            st["balance"] = bal_before
            ...
            try:
                ...
                bal_after_open = float(r.get("balance_after", bal_before - current_lot))
                ...
                if result:
                    ...
                    # ─── FIX KRITIS: sèvi _parsed_profit dirèkteman ───
                    buy_price  = result.get("_parsed_buy_price",  float(result.get("buy_price",  current_lot)))
                    sell_price = result.get("_parsed_sell_price", float(result.get("sell_price", 0)))
                    profit     = result.get("_parsed_profit",     sell_price - buy_price)
 
                    if status=="won":
                        pnl  = profit if profit > 0 else sell_price - buy_price
                        won  = True
                        ...
                        # "sold" oswa lòt — vèrifye balans reyèl
                        time.sleep(5)
                        nb = api.get_balance_sync()
                        if nb and nb > 0:
                            pnl = nb - bal_before; st["balance"] = nb; won = pnl > 0.01
                        else:
                            pnl = -(current_lot); won = False
                else:
                    # Timeout — vèrifye balans reyèl
                    time.sleep(5)
                    nb = api.get_balance_sync()
                    if nb and nb > 0:
                        pnl = nb - bal_before; st["balance"] = nb; won = pnl > 0.01
                ...
                if won:
                    ...
                    if consec_losses<=4:
                        ...
                        add_log(st,f"🔄 Reset apre 4 pèt | Total:${total_lost:.2f} | Tann 90sek...","WARN")
                        current_lot=base_lot; consec_losses=0; total_lost=0.0
                        time.sleep(90)
                ...
                trade={"id":len(st["trades"])+1,"time":datetime.now().strftime("%H:%M:%S"),
                    ...
                    "status":"won" if won else "lost","regime":"—"}
                st["trades"].insert(0,trade)
                st["total_pnl"]+=pnl
 
                # ─── Limit check apre chak trade ───
                if _check_limits(st,cfg): break
...
# ═══════════════════════════════════════════════════════════
# ██  DERIV TRADING LOOP — PnL FIX KONPLÈ  ██
# ═══════════════════════════════════════════════════════════
def trading_loop(st,bot_id=None):
    ...
    while st["running"]:
        ...
        try:
            ...
            regime,_=market_regime(candles); adx_val,pdi_val,mdi_val=calc_adx_full(candles,14)
            ...
            if consec_losses>=MAX_LOSSES_BEFORE_PAUSE:
                ...
                if not mache_bon:
                    add_log(st,f"⏸ PÒZ APRE {consec_losses} PÈT | Mache:{regime}(ADX:{adx_val:.0f}) — Ap tann siyal... ({PAUSE_WAIT_SECS}sek)","WARN")
            ...
            if sig=="BUY"  and regime=="TRENDING_DN":
                add_log(st,f"⛔ REJTE BUY — Mache ap DESANN. {st_sig}/{ha_sig}","WARN"); time.sleep(tf); continue
            if sig=="SELL" and regime=="TRENDING_UP":
                add_log(st,f"⛔ REJTE SELL — Mache ap MONTE. {st_sig}/{ha_sig}","WARN"); time.sleep(tf); continue
            ...
            # ─── FIX KRITIS: pran balans ANVAN trade ───
            bal_before = api.get_balance_sync()
            if not bal_before or bal_before <= 0:
                bal_before = st["balance"]
            st["balance"] = bal_before
            ...
            try:
                ...
                if r.get("contract_id"):
                    ...
                    # ─── FIX: eseye wait_contract_result anvan pou PAT ───
                    result_poc = None
                    api_obj = api
                    if hasattr(api_obj, 'wait_contract_result'):
                        try:
                            result_poc = api_obj.wait_contract_result(cid, timeout=max(30, tf//2))
                        except: pass
 
                    if result_poc:
                        status = result_poc.get("status","")
                        buy_price  = result_poc.get("_parsed_buy_price",  float(result_poc.get("buy_price",  current_lot)))
                        sell_price = result_poc.get("_parsed_sell_price", float(result_poc.get("sell_price", 0)))
                        profit     = result_poc.get("_parsed_profit",     sell_price - buy_price)
                        if status=="won":
                            pnl = profit if profit > 0 else sell_price - buy_price
                            ...
                            pnl = -(buy_price)
                            st["balance"] = bal_open
                            add_log(st,f"❌ PÈDI -${buy_price:.2f} | Bal:${st['balance']:.2f}","WARN")
                        else:
                            # Pran balans reyèl
                            nb = api.get_balance_sync()
                            if nb and nb > 0:
                                st["balance"] = nb; pnl = nb - bal_before
                            else:
                                pnl = -(bal_before - bal_open)
                    else:
                        # Fallback: kalkile pa balans
                        bal_close = None
            ...
            if ok:
                if pnl>0:
                    ...
                    if consec_losses<MAX_LOSSES_BEFORE_PAUSE:
                        ...
                        next_lot=round((total_lost+base_lot)/0.95,2)
                        current_lot=max(0.5,min(next_lot,100.0))
                        add_log(st,f"🚨 3 PÈT AFILE! | Total:${total_lost:.2f} | Mise rekipere:${current_lot:.2f} | Ap tann mache...","WARN")
                trade={
                    "id":len(st["trades"])+1,"time":datetime.now().strftime("%H:%M:%S"),
...
def binance_trading_loop(st,bot_id=None):
    ...
    if strategy=="binance_gold" or is_gold:
        ...
        add_log(st,f"🪙 Crypto Mode | {symbol}")
    elif strategy=="confluence":
        fn=lambda c: strat_confluence_binance(c,symbol)
        add_log(st,f"🔥 Confluence Binance | {symbol}")
    else:
        fn=STRATEGIES.get(strategy,strat_confluence_elite)
        add_log(st,f"📊 {strategy} | {symbol}")
    ...
    while st["running"]:
        ...
        try:
            ...
            if len(candles)<50:
                add_log(st,f"Pa ase done ({len(candles)}) — tann...","WARN"); time.sleep(60); continue
            ...
            if sig=="NONE" or conf<min_conf:
                add_log(st,f"⏭ Siyal fèb ({conf:.0%}) — tann pwochen bouji..."); time.sleep(tf); continue
            if e200_v:
            ...
            entry=candles[-1]["close"]
            sl_dol=round(current_lot*SL_PCT,2); tp_dol=round(current_lot*TP_PCT,2)
            ...
            try:
                ...
                if abs(pnl_chk)>0.01:
                    add_log(st,f"💹 Balans ajou: ${bal_after:.2f} ({'+' if pnl_chk>=0 else ''}{pnl_chk:.4f})")
                    if st["trades"]:
...
def api_connect():
    ...
    try:
        ...
        if broker=="deriv":
            ...
            add_log(st,f"🔑 {tok_type} token → koneksyon...")
            ok,balance,loginid,used_app_id,note=connect_deriv_token(raw_token,app_id)
            if not ok:
                return jsonify({"ok":False,"error":f"❌ Koneksyon echwe\n\n{note}"})
...
<div id="pg-dashboard" class="pg on">
    ...
    <div class="box">
      ...
      <div id="fd">
        <div style="background:#020C12;border:1px solid #0D2233;border-radius:8px;padding:12px;margin-bottom:12px">
          <div style="color:#00FF88;font-size:10px;font-weight:700;margin-bottom:8px">✅ PnL FIX + underlying_symbol FIX</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div style="background:#00FF8810;border:1px solid #00FF8830;border-radius:6px;padding:8px">
              ...
              <div style="color:#4A7080;font-size:9px;line-height:1.8">PA kòmanse ak <code>pat_</code><br>→ WebSocket ws.derivws.com<br>proposal: "symbol" ✅<br>PnL: balans diff ✅</div>
            </div>
            <div style="background:#FFD60010;border:1px solid #FFD60030;border-radius:6px;padding:8px">
              ...
              <div style="color:#4A7080;font-size:9px;line-height:1.8">→ REST Bearer + OTP WS<br>proposal: "underlying_symbol" ✅<br>PnL: sell_price-buy_price ✅<br>Limit profit/pèt ✅</div>
            </div>
          </div>
        </div>
        <div class="iw">
  ...
  <div class="box" style="background:#00FF8808;border-color:#00FF8822">
    <div class="bt" style="color:#00FF88">🚀 FIX KONPLÈ v6 — PnL KÒRÈK + STRATEGIES ELITE</div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;font-size:11px;color:#4A7080;line-height:1.9">
      <div>
        ...
        sell_price - buy_price<br>Bal pran ANVAN trade<br>
        <span style="color:#00FF88">Limit profit/pèt ✅</span><br>
        wait_contract_result ✅<br>
        <span style="color:#00FF88">→ PnL kòrèk nèt</span>
      </div>
      <div>
        <div style="color:#FFD600;font-weight:700;margin-bottom:4px">📈 SuperTrend</div>
        ATR x3.0 multiplier<br>Pwa: 2.5x (pi wo)<br>
        <span style="color:#FFD600">Travèse = siyal solid</span><br>
        Trend klè BUY/SELL<br>
        <span style="color:#FFD600">→ Pi reliable</span>
      </div>
      <div>
        <div style="color:#00D4FF;font-weight:700;margin-bottom:4px">🕯 Heikin Ashi</div>
        5 bouji konsekitif<br>Filtre bwi mache<br>
        <span style="color:#00D4FF">Pwa: 2.5x (pi wo)</span><br>
        Grandi = plis konfidans<br>
        <span style="color:#00D4FF">→ Trend pi klè</span>
      </div>
      <div>
        <div style="color:#FF3B6B;font-weight:700;margin-bottom:4px">🛡 ADX 12 + 3 strat</div>
        ADX sèyil: 12<br>3 strategies dakò min<br>Limit profit/pèt ✅<br>Pause 3 pèt konsève<br>
        <span style="color:#FF3B6B">→ Pwoteksyon kapital</span>
      </div>
    </div>
  </div>
</div>
...
<div id="pg-control" class="pg">
  <div class="g2">
    ...
    <div>
      ...
      <div class="box" style="background:#00FF8808;border-color:#00FF8822">
        <div class="bt" style="color:#00FF88">✅ PnL FIX — KÒMAN SA FONKSYONE</div>
        <div style="color:#4A7080;font-size:10px;line-height:2.1">
          <span style="color:#00FF88">✓ Bal ANVAN trade:</span> pran reyèl anvan<br>
          <span style="color:#00FF88">✓ WON:</span> sell_price - buy_price (reyèl)<br>
          <span style="color:#00FF88">✓ LOST:</span> -(buy_price) kòrèk<br>
          <span style="color:#FFD600">✓ Limit profit:</span> kanpe bot si rive<br>
          <span style="color:#FF3B6B">✓ Limit pèt:</span> kanpe bot si rive<br>
          <span style="color:#00D4FF">✓ Chak trade:</span> vèrifye limit apre
        </div>
      </div>
    </div>
  </div>
...
function autoDetectToken(){
  ...
  if(val.startsWith("pat_")){
    badge.style.display="inline";badge.className="bp";badge.textContent="⚡ PAT→underlying_symbol+PnL✅";
    hint.innerHTML='<span style="color:#FFD600">⚡ PAT → OTP WS → PnL kòrèk (sell-buy) ✅</span>';
  }else if(val.length>10){
    badge.style.display="inline";badge.className="bk";badge.textContent="✅ KLASIK→symbol+PnL✅";
    hint.innerHTML='<span style="color:#00FF88">✅ Token Klasik → WS → PnL kòrèk ✅</span>';
  }else{
...
const SI={
  confluence:{l:"🔥 Confluence ELITE",d:"v6: SuperTrend(2.5x)+HeikinAshi(2.5x)+Chandelier(2.5x)+10 strategies. ADX≥12, 3 strat min. PnL kòrèk ✅",tags:["SuperTrend","HeikinAshi","Chandelier","ADX≥12","PnL Fix"]},
  deriv_pro:{l:"🚀 Deriv Pro ELITE",d:"Score 5.0/15 + ADX≥12 + SuperTrend bonus. PnL kòrèk ✅",tags:["score 5/15","ADX≥12","ST bonus","PnL Fix"]},
  supertrend:{l:"📈 SuperTrend",d:"ATR × 3.0. Siyal klè BUY/SELL. Travèse bann = signal solid.",tags:["ATR×3","travèse=BUY","conf 75-92%"]},
  heikin_ashi:{l:"🕯 Heikin Ashi",d:"5 bouji konsekitif menm direksyon = trend solid.",tags:["5 bouji","filtre bwi","conf 72-83%"]},
  chandelier:{l:"🔔 Chandelier Exit",d:"Highest High - ATR×3. Chanjman trend detekte.",tags:["HH-ATR×3","LL+ATR×3","conf 75-90%"]},
  ai:{l:"🤖 AI Score",d:"8 faktè: EMA+RSI+MACD+BB+momentum+volatilite+position+trend.",tags:["8 faktè","pwa","conf 68-92%"]},
  smc:{l:"🏛 SMC",d:"Break of Structure + swing + EMA50.",tags:["BOS","swing","EMA50","conf 84%"]},
  scalping_pro:{l:"⚡ Scalping",d:"EMA 5/13 + RSI 9. Rapid pou 1m/5m.",tags:["EMA 5/13","RSI 9","rapid"]},
  rsi:{l:"📉 RSI",d:"RSI <30/>70 + EMA50 filtre.",tags:["RSI 14","OB 70","OS 30"]},
  binance_gold:{l:"🥇 Gold Strategy",d:"EMA200+ADX>20+Volume+RSI+MACD+BB+Stoch+SuperTrend. 7+ pts.",tags:["EMA 20/50/200","ADX>20","Volume","7+ pts"]},
  binance_crypto:{l:"🪙 Crypto Strategy",d:"EMA200+ADX>18+Volume surge+RSI+MACD+BB+Breakout+SuperTrend. 8+ pts.",tags:["EMA 9/21/50/200","ADX>18","Volume surge","8+ pts"]},
};
let sel="confluence";
...
async function doConn(){
  ...
  if(br=="deriv"){
    ...
    msg("cm",`⏳ ${isPat?"PAT → OTP WS | PnL kòrèk ✅":"Klasik → WS | PnL kòrèk ✅"} | Ap konekte...`,"ok");
  }
  ...
  try{
    ...
    if(d.ok){
      ...
      if(d.token_type){const hb=document.getElementById("h-tok-type");hb.style.display="inline";hb.className=d.token_type==="PAT"?"bp":"bk";hb.textContent=d.token_type==="PAT"?"⚡ PAT-OTP-PnL✅":"✅ Klasik-WS-PnL✅";}
...
async function doStart(){
  ...
  const d=await r.json();
  if(d.ok){msg("ctm","✓ BonheurBot ELITE v6 démarre! PnL Fix aktif ✅","ok");document.getElementById("bs").style.display="none";document.getElementById("bx").style.display="inline-block";}
...
if __name__=="__main__":
    ...
    logger.info(f"BonheurBot ELITE v6 — PnL FIX KONPLÈ — port {port}")
